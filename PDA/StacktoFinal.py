...
def main(automata, defAutomata):
    ...
    for i in range(0,n_rules):
        aux = rules[i] # q0 0 z0 q0 0,z0 | q1 0 1 q1 E
        aux2 = aux[4].split(",") #[0 z0] | [E]
        aux2.insert(0,aux[3]) # [q0 0 z0]| [q1, E]
        del(aux[4]) # q0 0 z0 q0         | q1 0 1 q1 
        del(aux[3]) # q0 0 z0            | 
        aux.append(aux2) # [q0 0 z0 [q0 0 z0 ]] | [q1 0 1 [q1, E]]
        rules[i] = aux #[q0 0 z0 [q0 0 z0 ]]

    ## adding wrappers ##
    for s in states:
        newRule = [str(s), 'E', 'x0', ['p', 'E']]
        rules.append(newRule)
        n_rules = n_rules + 1
    states.append('p')
    states.append('p0')
    newRuleStart = ['p0','E', stack[0], [lines[3].split()[0], stack_symbol[0], 'x0',]]
    rules.append(newRuleStart)
    n_rules = n_rules + 1
    for i in range(0,n_rules):
        print(rules[i])
    ...
    register = []
    getMoves(initial_state,automata)
...
main('E10',"./automata_empty_definition.txt")
